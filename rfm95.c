...
static void rfm95_write_fifo(const uint8_t *data, uint8_t length) {
    gpio_put(cs_pin, 0);
    uint8_t reg = RFM95_REG_FIFO | 0x80;
...
static void rfm95_read_fifo(uint8_t *data, uint8_t length) {
    gpio_put(cs_pin, 0);
    uint8_t reg = RFM95_REG_FIFO & 0x7F;
    spi_write_blocking(lora_spi, &reg, 1);
...
void rfm95_init(spi_inst_t *spi, uint cs, uint rst, uint irq) {
    ...
    rfm95_write_register(RFM95_REG_OP_MODE, RFM95_MODE_SLEEP);
    sleep_ms(10);
...
void rfm95_config(float freq, int tx_power) {
    ...
    // Modo LoRa
    rfm95_write_register(RFM95_REG_OP_MODE, RFM95_LONG_RANGE_MODE);
    sleep_ms(10);
    
    // Modo standby
    rfm95_write_register(RFM95_REG_OP_MODE, RFM95_LONG_RANGE_MODE | RFM95_MODE_STDBY);
    sleep_ms(10);

    // Configurar frequência
    uint64_t frf = (uint64_t)((freq * 1000000.0) / 61.03515625);
    rfm95_write_register(RFM95_REG_FRF_MSB, (uint8_t)(frf >> 16));
    rfm95_write_register(RFM95_REG_FRF_MID, (uint8_t)(frf >> 8));
    rfm95_write_register(RFM95_REG_FRF_LSB, (uint8_t)(frf >> 0));
    ...
    if (tx_power > 20) {
        // PA_BOOST + PA_DAC
        rfm95_write_register(0x4D, 0x87); // PA_DAC
        rfm95_write_register(RFM95_REG_PA_CONFIG, 0x80 | (tx_power - 5));
    } else {
        // PA_BOOST normal
        rfm95_write_register(0x4D, 0x84); // PA_DAC
        rfm95_write_register(RFM95_REG_PA_CONFIG, 0x80 | (tx_power - 5));
    }

    // Configurar modem (BW=125kHz, CR=4/5, SF=7)
    rfm95_write_register(RFM95_REG_MODEM_CONFIG1, 0x72); // BW=125kHz, CR=4/5, ImplicitHeader=off
    rfm95_write_register(RFM95_REG_MODEM_CONFIG2, 0x74); // SF=7, TxContinuousMode=off, CRC=on
    rfm95_write_register(RFM95_REG_MODEM_CONFIG3, 0x04); // LowDataRateOptimize=off, AGC=on

    // Configurar preâmbulo
    rfm95_write_register(RFM95_REG_PREAMBLE_MSB, 0x00);
    rfm95_write_register(RFM95_REG_PREAMBLE_LSB, 0x08);

    // Configurar endereços base do FIFO
    rfm95_write_register(RFM95_REG_FIFO_TX_BASE_ADDR, 0x00);
    rfm95_write_register(RFM95_REG_FIFO_RX_BASE_ADDR, 0x00);
...
void rfm95_send_message(const char *msg) {
    ...
    if (length > 63) length = 63; // Limitar tamanho
    
    // Configurar ponteiro do FIFO para base TX
    rfm95_write_register(RFM95_REG_FIFO_ADDR_PTR, 0x00);
    
    // Escrever dados no FIFO
    rfm95_write_fifo((const uint8_t*)msg, length);
    
    // Configurar tamanho do payload
    rfm95_write_register(RFM95_REG_PAYLOAD_LENGTH, length);
    
    // Configurar DIO0 para TxDone
    rfm95_write_register(RFM95_REG_DIO_MAPPING1, 0x40);
    
    // Limpar flags de interrupção
    rfm95_write_register(RFM95_REG_IRQ_FLAGS, 0xFF);
    ...
    while ((time_us_32() - start) < 1000000) {
        uint8_t irq_flags = rfm95_read_register(RFM95_REG_IRQ_FLAGS);
    ...
    // Limpar flag TxDone
    rfm95_write_register(RFM95_REG_IRQ_FLAGS, RFM95_IRQ_TX_DONE);
...
bool rfm95_receive_message(rfm95_packet_t *packet) {
    ...
    if (irq_flags & RFM95_IRQ_RX_DONE) {
        ...
        uint8_t length = rfm95_read_register(RFM95_REG_RX_NB_BYTES);
        
        if (length > 0 && length < 64) {
            // Obter endereço atual do FIFO RX
            uint8_t fifo_addr = rfm95_read_register(RFM95_REG_FIFO_RX_CURRENT_ADDR);
            rfm95_write_register(RFM95_REG_FIFO_ADDR_PTR, fifo_addr);
...
void rfm95_set_mode_rx(void) {
    ...
    rfm95_write_register(RFM95_REG_DIO_MAPPING1, 0x00);
    ...
    // Modo RX contínuo
    rfm95_write_register(RFM95_REG_OP_MODE, RFM95_LONG_RANGE_MODE | RFM95_MODE_RXCONTINUOUS);
}

void rfm95_set_mode_tx(void) {
    rfm95_write_register(RFM95_REG_OP_MODE, RFM95_LONG_RANGE_MODE | RFM95_MODE_TX);
}
...
int16_t rfm95_get_rssi(void) {
    int16_t rssi = rfm95_read_register(RFM95_REG_PKT_RSSI_VALUE);
...
int8_t rfm95_get_snr(void) {
    return (int8_t)rfm95_read_register(RFM95_REG_PKT_SNR_VALUE) / 4;
}